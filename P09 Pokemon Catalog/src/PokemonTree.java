...
public class PokemonTree {
  ...
  public static boolean addPokemonHelper(Pokemon newPokemon, PokemonNode current) {
    if (newPokemon.compareTo(current.getPokemon()) > 0 && current.getRightChild() == null) {
      ...
      return true;
    } else if (newPokemon.compareTo(current.getPokemon()) > 0 && current.getRightChild() != null) {
  ...
  public boolean addPokemon(Pokemon newPokemon) {
    ...
    if (root == null) { // Add new to an empty PokemonTree
      this.root = new PokemonNode(newPokemon);
      size++;
      return true;
    } else { // Add new to an non-empty PokemonTree
      if (addPokemonHelper(newPokemon, this.root)) {
        size++;
        return true;
      }
      return false;
  ...
  public static String toStringHelper(PokemonNode current) {
    ...
    String right;
    if (current != null) {
      left = toStringHelper(current.getLeftChild());
      middle = current.getPokemon().toString() + "\n";
      right = toStringHelper(current.getRightChild());
      currentTree = left + middle + right;
    }
    return currentTree;
  }
  ...
  public static Pokemon lookupHelper(int cp, PokemonNode current) {
    if (current != null) {
      ...
      } else if (cp > current.getPokemon().getCP() && current.getRightChild() != null) {
        return lookupHelper(cp, current.getRightChild());
      } else if (cp < current.getPokemon().getCP() && current.getLeftChild() != null) {
